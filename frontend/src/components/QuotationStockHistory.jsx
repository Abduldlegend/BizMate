  ...
  return (
    <div className="p-4">
      ...
      {!loading && groups.length === 0 && (
        <div className="text-gray-600">No quotations or stock lists yet.</div>
      )}

      <div className="space-y-4">
        {groups.map(group => (
          <div key={group.key} className="bg-white rounded-xl shadow-sm overflow-hidden">
            <button onClick={()=>toggleGroup(group.key)} className="w-full flex items-center justify-between p-3 md:p-4">
              <div className="flex items-center gap-3">
                <div className="text-sm md:text-base font-semibold">{group.label}</div>
                <div className="text-xs text-gray-500">{group.items.length} item{group.items.length>1? 's':''}</div>
              </div>
                ...
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }}>
                  <div className="p-3 md:p-4 border-t">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {group.items.map(doc => (
                        <div key={doc._id} className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg">
                          <div className="flex-1">
                            <div className="flex items-center justify-between">
                              <div>
                                <div className="text-sm font-semibold">{doc.businessName || '—'}</div>
                                <div className="text-xs text-gray-600">{doc.__type} • {format(new Date(doc.createdAt || doc.updatedAt), 'dd MMM yyyy, hh:mm a')}</div>
                              </div>
                              <div className="text-sm font-bold">{doc.grandTotal ? `₦${Number(doc.grandTotal).toFixed(2)}` : '-'}</div>
                            </div>

                            <div className="mt-2 text-sm text-gray-700 max-h-24 overflow-auto">
                              {doc.items && doc.items.length ? (
                                <ul className="list-disc pl-5">
                                  {doc.items.slice(0,4).map((it, i)=> (
                                    <li key={i} className="text-sm">{it.itemName} — {it.quantity} {it.unit} {it.price? `@ ₦${it.price}` : ''}</li>
                                  ))}
                                  {doc.items.length>4 && <li className="text-xs text-gray-500">and {doc.items.length-4} more...</li>}
                                </ul>
                          ...
                          <div className="flex flex-col items-center gap-2 ml-2">
                            <button onClick={()=>handleView(doc)} className="p-2 rounded-md bg-white border hover:shadow-sm" title="View"><Eye size={18} /></button>
                            <button onClick={()=>handleEdit(doc)} className="p-2 rounded-md bg-white border hover:shadow-sm" title="Edit"><Edit2 size={18} /></button>
                            <button onClick={()=>handleDelete(doc)} className="p-2 rounded-md bg-white border hover:shadow-sm text-red-600" title="Delete"><Trash2 size={18} /></button>
                          </div>
                        </div>
                      ))}
                    </div>
      ...
      <AnimatePresence>
        {viewing && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
            <motion.div initial={{ y: 20 }} animate={{ y: 0 }} exit={{ y: 20 }} className="w-full max-w-3xl bg-white rounded-2xl p-4">
              <div className="flex items-start justify-between">
                <div>
                  <div className="text-lg font-bold">{viewing.businessName || 'BizMate'}</div>
                  <div className="text-sm text-gray-600">{viewing.__type} • {format(new Date(viewing.createdAt || viewing.updatedAt), 'dd MMM yyyy, hh:mm a')}</div>
                </div>
                <div>
                  <button onClick={()=>setViewing(null)} className="text-sm text-gray-600">Close</button>
                </div>
              </div>
              ...
              <div className="mt-4">
                    ...
                    <tbody>
                      {(viewing.items || []).map((it, idx)=> (
                        <tr key={idx} className="border-b">
                          ...
                          <td className="p-2">{it.total ? `₦${it.total}` : '-'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div className="mt-4 text-right font-bold">Grand Total: {viewing.grandTotal ? `₦${Number(viewing.grandTotal).toFixed(2)}` : '-'}</div>
              </div>
            </motion.div>
          </motion.div>
        )}
