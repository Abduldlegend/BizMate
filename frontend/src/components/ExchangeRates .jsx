...
const ExchangeRates = () => {
  ...
  // Convert function
    ...
    <div className="max-w-6xl mx-auto bg-white shadow-lg rounded-2xl p-6 my-6">
  ...
  <div className="bg-gray-50 p-5 rounded-xl mb-8">
    ...
    <div className="flex flex-col lg:flex-row items-center gap-4">
      <input
        ...
        value={amount}
        onChange={(e) => setAmount(e.target.value)}
        className="border rounded-lg px-3 py-2 w-full lg:w-32"
      />
      <select
        value={fromCurrency}
        onChange={(e) => setFromCurrency(e.target.value)}
        className="border rounded-lg px-3 py-2 w-full lg:w-40"
      >
        {Object.keys(symbols).map((code) => (
          <option key={code} value={code}>
            {code} - {symbols[code].description}
    ...
    {converted !== null && (
      <p className="mt-4 text-lg font-bold text-green-600 text-center lg:text-left">
        {amount} {fromCurrency} = {converted.toFixed(2)} {toCurrency}
      </p>
    )}
  </div>
  ...
  <div>
    ...
    <select
      value={base}
      onChange={(e) => setBase(e.target.value)}
      className="border rounded-lg px-3 py-2 mb-5 w-full sm:w-60"
    >
      {Object.keys(symbols).map((code) => (
        <option key={code} value={code}>
          {code} - {symbols[code].description}
