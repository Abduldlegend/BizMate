    ...
    <div className='flex min-h-screen'>
      ...
      <main className='p-6 flex-1'>
            ...
            <tbody>
              {items.map((r, i)=>(
                <tr key={i} className='border-b'>
                  ...
                  <td className='p-2'><Input type='number' value={r.price} onChange={e=>update(i,'price', e.target.value)} /></td>
                  <td className='p-2 font-semibold'>{Number(r.total).toFixed(2)}</td>
        ...
        <div className='flex gap-3 mt-4'>
          ...
          <div className='ml-auto font-bold'>Grand Total: {grand.toFixed(2)}</div>
        </div>
        ...
        {preview && (
          <div className='mt-6 bg-white rounded-2xl shadow p-4'>
            ...
            <ul className='space-y-1'>
              {items.map((r,i)=>( <li key={i} className='text-sm'>{r.itemName} — {r.quantity} {r.unit} @ {r.price} = <b>{Number(r.total).toFixed(2)}</b></li> ))}
            </ul>
